...
from nnBuildUnits import CrossEntropy3d, topK_RegLoss, RelativeThreshold_RegLoss, gdl_loss, adjust_learning_rate, \
...
parser.add_argument("--lambda_D_WGAN_GP", default=10, type=float,
                    help="weight for gradient penalty of WGAN-GP, Default: 10")
parser.add_argument("--how2normalize", type=int, default=6, help="how to normalize the data")
parser.add_argument("--whichLoss", type=int, default=1,
                    help="which loss to use: 1. LossL1, 2. lossRTL1, 3. MSE (default)")
...
parser.add_argument("--whichNet", type=int, default=4,
                    help="which loss to use: 1. UNet, 2. ResUNet, 3. UNet_LRes and 4. ResUNet_LRes (default, 3)")
...
parser.add_argument("--numOfChannel_singleSource", type=int, default=5,
...
parser.add_argument("--decLREvery", type=int, default=10000,
                    help="Sets the learning rate to the initial LR decayed by momentum every n iterations, Default: n=40000")
parser.add_argument("--lrDecRate", type=float, default=0.5,
                    help="The weight for decreasing learning rate of netG Default=0.5")
parser.add_argument("--lrDecRate_netD", type=float, default=0.1,
                    help="The weight for decreasing learning rate of netD. Default=0.1")
...
parser.add_argument("--test_input_file_name", default='sub13_mr.hdr', type=str,
                    help="the input file name for testing subject")
parser.add_argument("--test_gt_file_name", default='sub13_ct.hdr', type=str,
                    help="the ground-truth file name for testing subject")
...
def main():
    ...
    for iter in range(opt.start_epoch, opt.numofIters + 1):
        ...
        if opt.isAdLoss:
            ...
            lossG_D = opt.lambda_AD * criterion_bce(outputD,
        ...
        # print statistics
        running_loss = running_loss + lossG_G.data  # [0] ho rimosso solo questo perché la versione di pytorch è>0.3
        ...
        if iter % opt.showTrainLossEvery == 0:  # print every 2000 mini-batches
            ...
            if opt.isAdLoss:
                print('loss_real is ', loss_real.data[0], 'loss_fake is ', loss_fake.data[0], 'outputD_real is',
                      outputD_real.data[0])
                print('loss for discriminator is %f' % lossD.data[0])
        ...
        if iter % opt.showValPerformanceEvery == 0:  # test one subject
            ...
            print('lossG_G is %.5f.' % (lossG_G.data))  # tolto [0]
        ...
        if iter % opt.showTestPerformanceEvery == 0:  # test one subject
            ...
            if opt.how2normalize == 6:
                ...
                print('maxPercentPET: ', maxPercentPET, ' minPercentPET: ', minPercentPET, ' maxPercentCT: ',
                      maxPercentCT, 'minPercentCT: ', minPercentCT)
            ...
            if not opt.isMultiSource:
                ...
                matOut = testOneSubject_aver_res(matFA, matGT, [5, 64, 64], [1, 64, 64], [1, 32, 32], net,
                ...
                matGT = hpetnp
                print('matFA shape: ', matFA.shape, ' matGT shape: ', matGT.shape)
                matOut = testOneSubject_aver_res_multiModal(matFA, matCT, matGT, [5, 64, 64], [1, 64, 64], [1, 32, 32],
                                                            net,
                                                            opt.prefixModelName + '%d.pt' % iter)
                print('matOut shape: ', matOut.shape)
                if opt.how2normalize == 6:
